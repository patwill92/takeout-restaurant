...
export const renderReviewsButtonConditionally = ({item,showReviews})=>{
	return item.reviews.length?(<button style={{padding:"10px", margin:"15px"}} onClick={()=>showReviews(item.itemName)}>
...
export const sayThankYouFor6thOfSecondAfterSubmission = ({item})=>{
	return item.specialMessage?item.specialMessageValue?(<h1>{item.specialMessageValue}</h1>):(<h1>Thank You</h1>):null
}
	...
	setTimeout(() => {
          this.setState({
            menu:this.state.menu.map((itemValue)=>{
                return itemValue.itemName !== item ? itemValue: {...itemValue,specialMessage:false,specialMessageValue:""}
            })
        })
        }, time)
}
...
export function makeReviewsVisibleToTheUser(item){
	this.setState({
            menu:this.state.menu.map((itemValue)=>{
                return itemValue.itemName !== item ? itemValue: {...itemValue,showReviews:!itemValue.showReviews,showInput:false}
            })
        })
}
